...
Pulls a tiny static ``mt-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/mt-runtime.json
...
Opt-out: set ``MT_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "MT_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/mt-eli-mcp/releases/latest/download/mt-runtime.json.gz",
)
_CACHE_NAME = "mt-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "mt-eli-mcp-runtime (+https://github.com/matematicsolutions/mt-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("MT_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "mt-eli"
