...
from mt_eli_mcp.coverage import build_coverage
from mt_eli_mcp.server import mt_coverage


def _coverage():
    fn = getattr(mt_coverage, "fn", mt_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MT_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("mt_coverage" in line for line in lines), (
        "an audit entry was written but it does not name mt_coverage"
    )
